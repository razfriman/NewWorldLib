    ...
    public unsafe class Kraken
    {
        ...
        public byte[] Decompress(byte[] compressedInput, int uncompressedSize)
        {
            ...
            fixed (byte* scratchPtr = decoder.Scratch)
            {
                ...
                {
                    if (!DecodeStep(decoder, decompressedBufferPtr, destinationOffset, remainingBytes, sourceStart,
                        sourceLength, scratchPtr))
                    {
            ...
            int remainingDestinationBytes, byte* source, int sourceBytesleft, byte* scratch)
        {
            ...
            switch (decoder.Header.DecoderType)
            {
                case DecoderTypes.Mermaid:
                    numBytes = MermaidDecodeQuantum(destination + destinationOffset,
                        destination + destinationOffset + destinationBytesLeft, destination,
            ...
            long offset, byte* scratch, byte* scratchEnd, MermaidLzTable* lz)
        {
            ...
            numBytes = DecodeBytes(&scratchOut, source, sourceEnd, &decodeCount,
                (uint)Math.Min(scratchEnd - scratch, destinationSize), false, scratch, scratchEnd);
            ...
            if (off16Count == 0xFFFF)
            {
                ...
                numBytes = DecodeBytes(&offset16High, source, sourceEnd, &offset16HighCount,
                    (uint)Math.Min(scratchEnd - scratch, destinationSize >> 1), false, scratch, scratchEnd);
        ...
        private bool MermaidProcessLzRuns(int mode, byte* source, byte* sourceEnd, byte* destination,
            int destinationSize, long offset, byte* destinationEnd, MermaidLzTable* lz)
        {
            ...
            for (iteration = 0; iteration != 2; iteration++)
            {
                ...
                {
                    sourceCurrent = MermaidMode1(destination, destinationSizeCurrent, destinationEnd, destinationStart,
        ...
        private byte* MermaidMode1(byte* destination, int destinationSize, byte* destinationEndPtr,
            byte* destinationStart, byte* sourceEnd, MermaidLzTable* lz, int* savedDist, int startOff)
        {
